...
- Commiteado en `6fb65bc`.

## Verificado por el general en su propio uso real

Repitió la prueba él mismo en `vexillum-prueba` (sin que yo tocara nada): dispatchó una mission (cola/queue con tests), le pidió al commander que no volviera a chequear por su cuenta, y sin ningún prompt nuevo de su parte llegó el aviso automático (`Stop hook feedback` vía `<task-notification>` + `system-reminder` en la transcripción real). El commander investigó la mission sola, reportó, preguntó si aterrizar, y aterrizó + liberó el camp al aprobarlo - ciclo completo confirmado independientemente de mis propias pruebas.

**Pendiente, encontrado en esa misma transcripción**: el commander probó `vexillum status 317da972dd90ce7f` para inspeccionar la mission terminada - ese comando no existe. Se recuperó solo (corrió `vexillum --help`, vio los comandos reales, y pasó a inspeccionar el camp por `git log`/`git status` directamente) sin romper nada, pero es una señal de que `AGENTS.md` podría ser más explícito sobre cómo inspeccionar una mission ya terminada (qué comando usar, o que no hay un `status` y hay que mirar el camp/task file) en vez de dejar que el commander adivine un comando inexistente cada vez. No es urgente - el fallback ya funciona bien - pero queda como mejora pendiente de `AGENTS.md`.
