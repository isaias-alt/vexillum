...
**Qué se hizo.** `vexillum doctor` reporta si el binario `no-mistakes` está instalado y si el proyecto ya corrió `no-mistakes init` (existe el remote git `no-mistakes`). Comando nuevo `vexillum ship <task-id>`: para una mission `done` (o `shipped` - reshippear para empujar más commits al mismo PR es válido), corre `git push no-mistakes <camp-branch>` desde el camp - determinista, sin juicio de agente de por medio. Si el proyecto no está gateado todavía, `ship` corre `no-mistakes init` él mismo la primera vez (decisión tomada con el general después de la primera pasada, ver `docs/no-mistakes.md` "Auto-gate en ship, no en init"): el setup "viene por defecto" sin que nadie tenga que acordarse de correrlo aparte, pero nada de red pasa hasta que el general efectivamente pide shippear algo - `vexillum init` se queda puramente local. No supervisa el pipeline después de eso.

**Extensión posterior (reconciliación camp/PR).** Encontrado después de cerrar B.4 la primera vez: `ship` no dejaba ningún registro de que un task había sido shippeado, así que `land` podía hacer fast-forward de la branch vieja del camp si no-mistakes aplicaba auto-fixes en su propio worktree, y `release` nunca reconocía un PR mergeado con squash/rebase en GitHub como aterrizado (el chequeo de ancestro no ve más allá del commit de merge). Cerrado agregando `state.StatusShipped`: `land` sobre un task shippeado mergea el PR real vía `gh` (verificado en vivo, atado a `--match-head-commit`) en vez de tocar el camp, y `release` gana un fallback de comparación de contenido (`git merge-tree`, sin `gh`) para el caso squash/rebase - versión mínima de lo que ya resuelve `github.com/kunchenguid/firstmate` para el mismo problema. Detalle completo en `docs/no-mistakes.md`, sección "Reconciliación camp/PR después de ship".
...
- No entra: cualquier lógica de review/test/lint/docs en el binario Go (100% de `no-mistakes`); supervisión del pipeline post-push (el general usa las herramientas propias de `no-mistakes`); `no-mistakes init` desde `vexillum init` (vive en `ship`, lazy, en el primer uso); gh-axi, la AXI agent-ergonomic (no es una dependencia real de este flujo - distinto de `gh`, el CLI oficial, que sí se usa ahora para el merge de una mission shippeada); forges distintos de GitHub; manejo de merge de una mission `land`eada localmente (sigue siendo del general - la excepción es la mission shippeada, ver "Extensión posterior" arriba).
...
- **Lieutenant (orquestador secundario), local y remoto por SSH.** Cerrado, no diferido - ver ADR-10 (`docs/adr.md`). El cuello de botella del uso personal es la cuota del plan de Claude; un segundo commander, local o remoto, consume del mismo plan y no agrega capacidad, y el remoto exige proyectos solo-ship más una durabilidad de endpoint más cara de lo que estimaba ADR-09. Se reabre solo ante un límite distinto de la cuota, no porque la v2 (o cualquier parte de ella) esté cerrada. Diseño de referencia real en firstmate (`docs/remote-secondmates.md`, `docs/secondmate-parent-channel.md`) sigue vigente si se reabre: un firstmate home entero en otra máquina, primary dueño de routing y supervisión, secondmate dueño de su estado local, sin reconciliar estado entre pares.
...
- **Estado de una mission shippeada, no observado por Vexillum.** `StatusShipped` es terminal desde el punto de vista de Vexillum, pero el trabajo real (el pipeline de no-mistakes, sus auto-fixes, el PR) sigue vivo afuera - un segundo commander (lieutenant) no tiene forma de saber en qué quedó ese PR sin consultar `no-mistakes` directamente. Misma clase de frontera no observada que el browser huérfano de B.3 (arriba), pero para `no-mistakes` en vez de chrome-devtools-axi: trabajo vivo que el estado de Vexillum da por cerrado. No resolver haciendo que Vexillum observe el pipeline de no-mistakes - sería reimplementar la supervisión que B.4 ya decidió delegar.
