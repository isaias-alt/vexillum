...
// state.StatusBlocked (see RunInHerdr and internal/sentinel.tickProject,
// the only two places that transition happens). It's a heuristic, not a
// parser: herdr's "recent-unwrapped" transcript ends with whatever the
// agent most recently emitted, so the last paragraph is taken as the
// question, and any numbered/bulleted lines within it as its options. The
// result is always real, structured text on its own fields - never a
// pointer back into transcript prose for a caller to re-parse later.
//
